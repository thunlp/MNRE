...
vector<double> test(int idx, vector<int> &sentence, vector<int> &PositionE1, vector<int> &PositionE2, int len) {
	...
	for (int i = 0; i < 3 * dimensionC; i++) {
		...
		for (int i1 = -window+1; i1 < len; i1++) {
			if (i1>=0&&PositionE1[i1]==limit)
				i2++;
			if (i1>=0&&PositionE2[i1]==limit)
				i2++;
			float res = 0;
			int tot = 0;
			int tot1 = 0;
		...
		for (int i1 = 0; i1<3; i1++)
		{
			r.push_back(mx[i1]+matrixB1[idx][3*i+i1]);
		}
	}
			...
			for (int idx_att=0; idx_att<num_language; idx_att++)
			{
				...
				for (int j = 0; j < relationTotal; j++) {
					...
					for (int k=0; k<sent_num; k++)
					{
						float s = 0;
						for (int i = 0; i < 3 * dimensionC; i++) 
							s += rList[k][i] * embRelation[idx_att][j * 3 * dimensionC + i];
						s = exp(s); 
						weight.push_back(s);
						weight_sum += s;
					}
					...
					for (int j1 = 0; j1 < relationTotal; j1++) {
						...
						for (int i1 = 0; i1 < 3 * dimensionC; i1++)
							s +=  0.5 * 0.5 * (1 * embRelation[idx_att][j1 * 3 * dimensionC + i1] + 1 * matrixRelation[0][j1 * 3 * dimensionC + i1]) * r[i1];
...
{
	double correct=0;
	FILE* f = fopen(("out/pr"+version+"_Zh.txt").c_str(), "w");
	...
	for (int idx = 0; idx<num_language; idx++)
	{
		for (int i = 0; i < relationTotal; i++) {
			...
			{
				fprintf(fout, "%f\t", matrixRelation[0][i * 3 * dimensionC + j]);
				fprintf(fout, "%f\t", embRelation[idx][i * 3 * dimensionC + j]);
			}
			fprintf(fout, "\n");
		}
		for (int i = 0; i < relationTotal; i++) 
