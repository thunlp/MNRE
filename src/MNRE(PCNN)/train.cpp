...
//	cout<<32<<' '<<dimensionC<<endl;
	...
	//	cout<<i<<endl;
		for (int i1 = -window+1; i1 <len; i1++) {
			if (i1>=0&&trainPositionE1[i1]==limit)
				i2++;
			if (i1>=0&&trainPositionE2[i1]==limit)
				i2++;
			float res = 0;
			int tot = 0;
			int tot1 = 0;
		...
		{
			//cout<<mx[i1]<<' '<<ti[i1]<<' ';
			r.push_back(mx[i1]+matrixB1Dao[idx][3*i+i1]);
			tip.push_back(ti[i1]);
		}
	//	cout<<endl;
	}
...
float train_bags(int idx, string bags_name)
{
	...
	for (int idx_att = 0; idx_att<num_language; idx_att++)
	{
		...
		for (int k=0; k<sent_num; k++)
		{
			float s = 0;
			for (int i = 0; i < 3 * dimensionC; i++) 
				s += rList[k][i] * embRelationDao[idx_att][r1 * 3 * dimensionC + i];
			s = exp(s); 
			weight.push_back(s);
			weight_sum += s;
		}
		...
		for (int j = 0; j < relationTotal; j++) {	
			...
			float ss = 0, ss1=0;
			for (int i = 0; i < 3 * dimensionC; i++) {
				ss += dropout[i] * r[i] * 0.5 * matrixRelationDao[0][j * 3 * dimensionC + i];
				//ss1 += dropout[i] * r[i] * embRelationDao[idx_att][j * dimensionC + i];
				ss += dropout[i] * r[i] * 0.5 * embRelationDao[idx_att][j * 3 * dimensionC + i];
			}
		...
		{	
			...
			for (int i = 0; i < 3 * dimensionC; i++) 
			{
				...
				if (dropout[i]!=0)
				{
					g1 += 0.5 * g * matrixRelationDao[0][r2 * 3 * dimensionC + i];
					matrixRelation[0][r2 * 3 * dimensionC + i] -= 0.5 * g * r[i];
				//	if (turn<8)
					embRelation[idx_att][r2 * 3 * dimensionC + i] -= 0.5 * g * r[i];
					g1 += 0.5 * g * embRelationDao[idx_att][r2 * 3 * dimensionC + i];
	...
	//	cout<<197<<endl;
		for (int i = 0; i < 3 * dimensionC; i++) 
		{
			...
			for (int k=0; k<sent_num; k++)
			{
				...
				grad[k][i]+=g1*rList[k][i]*weight[k]*embRelationDao[idx_att][r1 * 3 * dimensionC + i];
				embRelation[idx_att][r1 * 3 * dimensionC + i]  += g1*rList[k][i]*weight[k]*rList[k][i];
				assert(embRelation[idx_att][r1 * 3 * dimensionC + i]==embRelation[idx_att][r1 * 3 * dimensionC + i]);
				tmp_sum += rList[k][i]*weight[k];
			}	
			for (int k1=0; k1<sent_num; k1++)
			{
				grad[k1][i]-=g1*tmp_sum*weight[k1]*embRelationDao[idx_att][r1 * 3 * dimensionC + i];
				embRelation[idx_att][r1 * 3 * dimensionC + i] -= g1*tmp_sum*weight[k1]*rList[k1][i];
				assert(embRelation[idx_att][r1 * 3 * dimensionC + i]==embRelation[idx_att][r1 * 3 * dimensionC + i]);
			}
		}
