...
void init_para()
{
	...
	for (int idx1 = 0; idx1<num_language; idx1++)
		embRelation[idx1].resize(dimensionC * relationTotal);
	...
	{
		matrixRelation[idx].resize(dimensionC * relationTotal);
		...
		matrixB1[idx].resize(dimensionC);
		...
		for (int i = 0; i < dimensionC; i++) {
			...
			matrixB1[idx][i] = getRandU(-con1, con1);
		}
		...
		for (int i = 0; i < relationTotal; i++) 
		{
			...
			for (int j = 0; j < dimensionC; j++)
				matrixRelation[idx][i * dimensionC + j] = getRandU(-con, con);
		}
...
void init(string language, int idx, int istest) {
	...
	if (istest==0)
		read_data(0, "train_"+language, wordMapping[idx], testheadList[idx], testtailList[idx], testrelationList[idx], testLength[idx], 
			testLists[idx], testPositionE1[idx], testPositionE2[idx], bags_test[idx]);
	else
	if (istest==2)
